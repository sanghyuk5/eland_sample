...
class EKidsRecommendCategoryViewHolder(
    ...
    private val tabClickCallback: ItemClickIntCallback = { index ->
        ...
        if (isWeeklyBest) {
            EventBus.fire(HolderEvent(HolderEventType.TAB_CLICK_WEEKLY, index.toString()))
        } else {
            EventBus.fire(HolderEvent(HolderEventType.TAB_CLICK_NEW_ARRIVAL, index.toString()))
        }
    }
